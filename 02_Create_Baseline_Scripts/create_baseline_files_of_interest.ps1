param([string] $FileOut="..\Baseline\Files_of_Interest_Baseline.csv", [bool] $ToScreen)
...
$output = $targets | ForEach-Object {if($_.Subnet -eq "OpsNet" -and $_.OS -eq "Win10") {
    Invoke-Command -UseSSL -SessionOption $so -ComputerName $_.IP -Credential $creds -FilePath ..\01_Reference_Scripts\filesofinterest.ps1 -ArgumentList (,$configs.file_patterns)}}
